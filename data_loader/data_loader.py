...
def data_loader():
    ...
    for i, action_type in enumerate(action_types[:-1]):  # except padding
        is_act = tf.reshape(
            ...
            [-1, hist_item_num, 1])
        act_time_emb = tf.reshape(
            k.nn.get_dense_fea(f"{action_type}_time_emb_list", dim=hist_item_num, dtype=tf.float32),
            [-1, hist_item_num, emb_size])

        action_emb = action_emb_dict[i, :][tf.newaxis, tf.newaxis, :]
        action_emb = tf.tile(
            action_emb,
            [tf.shape(is_act)[0], hist_item_num, 1]
        )
        time_action_emb = tf.concat([act_time_emb, action_emb], axis=-1)
        time_action_emb_list.append(tf.expand_dims(is_act * time_action_emb, axis=2))

    hist_action_time_emb_seq = tf.concat(time_action_emb_list, axis=2)
    ...
    for i in range(5):
        ...
        user_seq_embed = tf.reshape(user_seq_embed, [-1, other_seq_length, emb_size])
        user_other_seq_list.append(user_seq_embed)
    user_other_seq = tf.reduce_mean(tf.concat(user_other_seq_list, axis=2), axis=1)

    # ------------------------ label construct ------------------------ #
    label_action_seq = k.nn.get_dense_fea(f"action_seq", dim=len(action_types) - 1, dtype=tf.int32)
    label_time_seq = k.nn.get_dense_fea(f"time_seq", dim=len(action_types) - 1, dtype=tf.float32)
    label_time_emb_seq = k.nn.new_embedding(f"time_emb_seq", dim=len(action_types) - 1)
    label_time_emb_seq = tf.reshape(label_time_emb_seq, [-1, len(action_types), emb_size])
    label_action_type_onehot_seq = tf.one_hot(label_action_seq, axis=-1, depth=len(action_types))
    label_action_emb_seq = tf.matmul(label_action_type_onehot_seq, action_emb_dict)

    label_action_time_emb_seq = tf.concat([tf.expand_dims(label_time_emb_seq, axis=-1), label_action_emb_seq], axis=-1)
    # ------------------------------------------------------------------ #

    user_other_seq_output = mlp_block('seq_process', user_other_seq, [emb_size])
    user_emb = tf.concat([user_emb, user_other_seq_output], axis=-1)
